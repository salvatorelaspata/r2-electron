...
const updateDashboard = async () => {
    ...
    try {
        ...
        bucketsContainer.innerHTML = Object.entries(stats)
            .map(([bucketName, bucketStats]) => renderBucketCard(bucketName, bucketStats))
            .join('');

        document.querySelectorAll('.bucket-card').forEach(card => {
            card.addEventListener('click', () => handleBucketClick(card.dataset.bucket));
        });
        ...
        UI.showError(`Errore durante l'aggiornamento della dashboard: ${error.message}`);
...
const handleBucketClick = async (bucketName) => {
    ...
    try {
        ...
        console.error('Errore durante il caricamento dei dettagli del bucket:', error);
        UI.showError(`Errore durante il caricamento dei dettagli del bucket: ${error.message}`);
